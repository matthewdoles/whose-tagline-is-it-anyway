	...
	handle(handlerInput) {
		...
		return ms.getInSkillProduct(locale, productId).then(function (result) {
			if (result.entitled == 'ENTITLED') {
				...
				let speechText = "<voice name='" + voice_name + "'>Hello, which game would you like to play: Whose Tagline Is It Anyway, or Good Word Hunting</voice>";
				let repromptText = "<voice name='" + voice_name + "'>Sorry, I did not quite understand that. Would you like help?</voice>";
				...
				let speechText = "<voice name='" + voice_name + "'>Hello, welcome to Whose Tagline Is It Anyway. Would you like to play a quick round? Or, for more information, please say 'help'</voice>";
				let repromptText = "<voice name='" + voice_name + "'>Sorry, I did not quite understand that. If you would like more information, please say 'help'.</voice>";
				handlerInput.attributesManager.setSessionAttributes({ type: "whoseTagline" });
				return handlerInput.responseBuilder.speak(speechText).reprompt(repromptText).getResponse();
			}
		});
	}
...
const GoodWordHuntingIntent = {
	canHandle(handlerInput) {
		return (
			handlerInput.requestEnvelope.request.type === "IntentRequest" &&
			handlerInput.requestEnvelope.request.intent.name === "GoodWordHuntingIntent");
	},
	handle(handlerInput) {
		...
		return ms.getInSkillProduct(locale, productId).then(function (result) {
			if (result.entitled == 'ENTITLED') {
				...
				let speechText = "<voice name='" + voice_name + "'>Would you like to play Good Word Hunting with extended time?</voice>";
				let repromptText = "<voice name='" + voice_name + "'>Would you like to play Good Word Hunting with extended time?</voice>";
				handlerInput.attributesManager.setSessionAttributes({ type: "goodWordHuntingStart" });
				return handlerInput.responseBuilder.speak(speechText).reprompt(repromptText).getResponse();
			}
			else {
				// user not eligible to play, ask if they would like to hear game description
				let speechText = "<voice name='" + voice_name + "'>Sorry, it seems you haven't purchased the rights to play Good Word Hunting. Would you like to hear a little bit about this game?</voice>";
				let repromptText = "<voice name='" + voice_name + "'>Would you like to hear a little bit about Good Word Hunting?</voice>";
				handlerInput.attributesManager.setSessionAttributes({ type: "goodWordHuntingHelp" });
				return handlerInput.responseBuilder.speak(speechText).reprompt(repromptText).getResponse();
			}
		});
	}
	...
	async handle(handlerInput) {
		...
		if (attributes.movie_id == undefined) {
			try {
				...
				speechText = "<voice name='" + voice_name + "'>Sorry, an error occurred getting data from The Movie Database. Would you like to try again?</voice>";
				repromptText = "<voice name='" + voice_name + "'>Would you like to try again?</voice>";
				handlerInput.attributesManager.setSessionAttributes({ type: attributes.type });
				console.log(error);
				return handlerInput.responseBuilder.speak(speechText).reprompt(repromptText).getResponse();
			}
		}
		...
		if (attributes.type == 'whoseTagline') {
			try {
				...
				if (tagline) {
					speechText = "<voice name='" + voice_name + "'>Alright, the tagline for this movie is, '<break time='1s'/>" + tagline + "<break time='1s'/>'. I'll give you a few seconds to think on it. <break time='4s'/>" +
						"Okay, what would you like to do? Repeat the tagline, get a hint, or answer.</voice>";
					repromptText = "<voice name='" + voice_name + "'>What would you like to do? Repeat the tagline, get a hint, or answer.</voice>"
				...
				speechText = "<voice name='" + voice_name + "'>Sorry, an error occurred getting data from The Movie Database. Please try again.</voice>";
				console.log(error);
			}
		}
			...
			try {
				...
				if (random_movie_keywords.keywords.length != 0) {
					...
					// if more than 5 words available to describe movie, list the first 5
						...
						speechText = "<voice name='" + voice_name + "'>I'll give you 5 keywords used to describe this film. Which are <break time='1s'/>";
						for (let i = 0; i < 5; i++) {
							if (i == 4) {
								speechText += " and " + random_movie_keywords.keywords[i].name + ". <break time='1s'/>";
								keywords[i] = random_movie_keywords.keywords[i].name;
					...
					try {
						...
						if (random_movie_credits.cast.length != 0) {
							...
							if (random_movie_credits.cast.length < 10) {
								speechText += ">There are only " + random_movie_credits.cast.length + " cast members for this film. Out of these " + random_movie_credits.cast.length + " names, ";
						...
						return handlerInput.responseBuilder
							...
							.speak(speechText)
							.reprompt("<voice name='" + voice_name + "'>How many names do you need to name this movie?</voice>")
							.getResponse();
					}
					catch (error) {
						speechText = "<voice name='" + voice_name + "'>Sorry, an error occurred getting data from The Movie Database. Would you like to try again?</voice>";
	...
	async handle(handlerInput) {
		...
		if (attributes.type == "whoseTagline" || attributes.type == "answer") {
			...
			if (attributes.hint == 1) {
				speechText = "<voice name='" + voice_name + "'>This movie came out in " + attributes.year + ". Was that any help? I'll give you a few more seconds to think on it. <break time='4s'/>" +
			...
			// second hint, movie top two billed cast members, make api call
				...
				try {
					...
					speechText = "<voice name='" + voice_name + "'>Sorry, an error occurred getting data from The Movie Database for your hint. Would you like to answer?</voice>";
					console.log(error);
				}
			}
			// only two hints allowed, prompt for user to answer
			else {
				speechText = "<voice name='" + voice_name + "'>I'm sorry, I'm afraid that is all the hints I can give. Would you like to repeat the tagline or answer?</voice>";
			}
			handlerInput.attributesManager.setSessionAttributes({
				type: "answer",
			...
			return handlerInput.responseBuilder
				.speak(speechText)
				.reprompt("<voice name='" + voice_name + "'>Would you like to answer?</voice>")
				.getResponse();
		}
		// if game is good word hunting, secretly allow for no hints
		if (attributes.type == "goodWordHunting") {
			speechText = "<voice name='" + voice_name + "'>Sorry, I can't give you any hints, but I'll give you a few more seconds to think of your answer. <break time='4s'/>" +
			...
			if (easterEgg == 3) {
				speechText = "<voice name='" + voice_name + "'>Okay I give in, this movie came out in " + attributes.year + ". I hope that helps, I'll give you a few more seconds to think of your answer. <break time='4s'/>" +
					"Okay, what movie are these keywords and cast members associated with?</voice>";
			}
			// but no more hints after that!
			if (easterEgg > 3) {
				speechText = "<voice name='" + voice_name + "'>That is seriously all the hints I am going to give you. However, I'll still give you a few more seconds to think of your final answer. <break time='4s'/>" +
					"Okay, what movie are these keywords and cast members associated with?</voice>";
			}
...
const MovieCastIntent = {
	...
	handle(handlerInput) {
		...
		let numberNamesNeeded = handlerInput.requestEnvelope.request.intent.slots.numberNamesNeeded.value;
		let attributes = handlerInput.attributesManager.getSessionAttributes();
		let speechText = "";
		...
		if (isNaN(numberNamesNeeded) == true) {
			...
			return handlerInput.responseBuilder
				...
				.speak("<voice name='" + voice_name + "'>I'm sorry, I'm having trouble understanding your response. From lowest billed to highest, how many names do you think you need to name this film?</voice>")
				.reprompt("<voice name='" + voice_name + "'>From lowest billed to highest, how many names do you think you need to name this film?</voice>")
				.getResponse();
		}
		// if slot number is larger than cast size or smaller than zero, tell user to give a correct number
			...
			return handlerInput.responseBuilder
				...
				.speak("<voice name='" + voice_name + "'>I'm sorry, the number must be between 0 and " + attributes.cast.length + ". from lowest billed to highest, how many names do you think you need to name this film?</voice>")
				.reprompt("<voice name='" + voice_name + "'>From lowest billed to highest, how many names do you think you need to name this film?</voice>")
				.getResponse();
		}
		else {
			...
			if (numberNamesNeeded == 0) {
				speechText = "<voice name='" + voice_name + "'>You believe you can name this movie with no cast names give. What is the name of this movie?</voice>"

			}
			// speech text if user needs only one name
			else if (numberNamesNeeded == 1) {
				speechText = "<voice name='" + voice_name + "'>You said you need only one name in order to name this movie. That lowest billed name is " + attributes.cast[attributes.cast.length - 1] + "<break time='1s'/>. "
			...
			// speech text if user needs all names
				...
				speechText = "<voice name='" + voice_name + "'>You said you need all " + numberNamesNeeded + " names in order to name this movie. Those names from lowest to highest billed are <break time='1s'/>";
				...
				speechText += "With that, I'll give you a few more seconds to think about it. <break time='4s'/> Alright, what movie are these keywords and cast members associated with?</voice>";
			}
			// speech text if user needs in between two and cast length minus one names
			else {
				speechText = "<voice name='" + voice_name + "'>You said you need " + numberNamesNeeded + " out of " + attributes.cast.length + " names in order to name this movie. Those names from lowest to highest billed are <break time='1s'/>";
				let castIndex = attributes.cast.length - 1;
				...
				speechText += "With that, I'll give you a few more seconds to think of your answer. <break time='4s'/> Alright, what movie are these keywords and cast members associated with? If you would like to hear the keywords and cast members again say repeat.</voice>";
			}
			...
			return handlerInput.responseBuilder
				...
				.speak(speechText)
				.reprompt("<voice name='" + voice_name + "'>What movie are these keywords and cast members associated with?</voice>")
				.getResponse();
		}
	} 
};
const AnswerIntent = {
	...
	handle(handlerInput) {
		...
		return handlerInput.responseBuilder
			...
			.speak("<voice name='" + voice_name + "'>Alright, what movie is this the tagline for?</voice>")
			.reprompt("<voice name='" + voice_name + "'>Any guess is better than nothing.</voice>")
			.getResponse();
	}
};
const GameResultsIntent = {
	...
	handle(handlerInput) {
		...
		if (attributes.type == "goodWordHunting") {
			// recap keywords
			let speechText = "<voice name='" + voice_name + "'>Okay, to recap, the keywords for this movie where <break time='1s'/>'";
			...
			let numberOfNamesLeft = attributes.cast.length - attributes.numberNamesNeeded;
			...
			// prompt for new game
			handlerInput.attributesManager.setSessionAttributes({ type: "goodWordHunting" });
			speechText += "Thank you for playing, would you like to play another round?</voice>";
			return handlerInput.responseBuilder
				.speak(speechText)
				.reprompt("<voice name='" + voice_name + "'>To play another round, please say 'play Good Word Hunting'.</voice>")
				.getResponse();
		}
		// finalize game results for whose tagline is it anyway
		else {
			let speechText = "<voice name='" + voice_name + "'>Okay, to recap, the tagline was <break time='1s'/>'" + attributes.tagline + "'<break time='1s'/>. Your guess was, '" + guess +
			"'. And the answer is, <break time='1s'/> '" + attributes.movie + "' <break time='1s'/>. Thank you for playing. Would like to play another round?</voice>";
			handlerInput.attributesManager.setSessionAttributes({ type: "whoseTagline" });
			return handlerInput.responseBuilder
				.speak(speechText)
				.reprompt("<voice name='" + voice_name + "'>To play another round, please say 'play Whose Tagline Is It Anyway'.</voice>")
				.getResponse();
...
const GetTaglineIntent = {
	...
	async handle(handlerInput) {
		...
		try {
			...
			if (search_movie.results.length > 0) {
				...
				try {
					...
					speechText = "<voice name='" + voice_name + "'>The tagline for " + original_title + " from " + year.substring(0, 4) + " is <break time='1s'/>" + tagline + ". </voice>";
				}
				catch (error) {
					speechText = "<voice name='" + voice_name + "'>Sorry, an error occurred getting data from The Movie Database. Please try again. </voice>";
					console.log(error);
			...
			// if no result found, reprompt user to include the year of the movie after the title
				...
				speechText = "<voice name='" + voice_name + "'>Sorry, I was not able to find a match for " + movie + ". Try including the year after the title by saying " +
		...
		speechText += "<voice name='" + voice_name + "'>If you would like to get the tagline for another movie, please say get tagline for 'insert movie'.</voice>";
		return handlerInput.responseBuilder.speak(speechText).reprompt("<voice name='" + voice_name + "'>If you would like to get the tagline for another movie, please say get tagline for 'insert movie'.</voice>").getResponse();
	}
};
const GetMovieCastIntent = {
	...
	async handle(handlerInput) {
				...
				try {
					...
					if (search_movie_credits.cast.length != 0) {
						...
						if (search_movie_credits.cast.length < 10) {
							speechText = "<voice name='" + voice_name + "'>There are only " + search_movie_credits.cast.length + " cast members for " + movie + " from " + year.substring(0, 4) + ". These " + search_movie_credits.cast.length + " names, from highest billed to lowest are <break time='1s'/>";
							for (let i = 0; i < search_movie_credits.cast.length; i++) {
								if ((i + 1) == search_movie_credits.cast.length) {
									speechText += "and " + search_movie_credits.cast[i].name + ". <break time='1s'/></voice>";
								}
								else {
									speechText += search_movie_credits.cast[i].name + ", <break time='1s'/>";
								}
							}
						}
						// if movie has less than 10 credits, name them all from highest billed to lowest
						else {
							speechText = "<voice name='" + voice_name + "'>There are over 10 cast members for " + movie + " from " + year.substring(0, 4) + ". The top 10 billed names, from highest billed to lowest are <break time='1s'/>";
							for (let i = 0; i < 10; i++) {
								if ((i + 1) == 10) {
									speechText += "and " + search_movie_credits.cast[i].name + ". <break time='1s'/></voice>";
								}
		...
		speechText += "<voice name='" + voice_name + "'>If you would like to get the cast for another movie, please say get cast for 'insert movie'.</voice>";
		return handlerInput.responseBuilder.speak(speechText).reprompt("<voice name='" + voice_name + "'>If you would like to get the cast for another movie, please say get cast for 'insert movie'.</voice>").getResponse();
	}
};
const HelpIntent = {
	canHandle(handlerInput) {
	...
	handle(handlerInput) {
		...
		let speechText = "<voice name='" + voice_name + "'>For help and information about Whose Tagline Is It Anyway, please say 'help with Whose Tagline Is It Anyway'. " +
		...
		return handlerInput.responseBuilder.speak(speechText).reprompt("<voice name='" + voice_name + "'>What area can I help you with?</voice>").getResponse();
	}
};
const HelpWhoseTaglineIntent = {
	canHandle(handlerInput) {
		return (
			handlerInput.requestEnvelope.request.type === "IntentRequest" &&
			handlerInput.requestEnvelope.request.intent.name === "HelpWhoseTaglineIntent"
		);
	},
	handle(handlerInput) {
		console.log("HelpWhoseTaglineIntent Handler Called");
		// help option explaining how to play Whose Tagline Is It Anyway 
		let speechText = "<voice name='" + voice_name + "'>In Whose Tagline Is It Anyway, you will be given the tagline for a random movie. With that tagline, " +
		...
		handlerInput.attributesManager.setSessionAttributes({ type: "whoseTagline" });
		return handlerInput.responseBuilder.speak(speechText).reprompt("<voice name='" + voice_name + "'>Would you like to play a quick round?</voice>").getResponse();
	}
};
const HelpGoodWordHuntingIntent = {
	canHandle(handlerInput) {
		return (
			handlerInput.requestEnvelope.request.type === "IntentRequest" &&
			handlerInput.requestEnvelope.request.intent.name === "HelpGoodWordHuntingIntent"
		);
	},
	handle(handlerInput) {
		console.log("HelpGoodWordHuntingIntent Handler Called");
		// help option explaining how to play Good Word Hunting
		let speechText = "<voice name='" + voice_name + "'>Good Word Hunting is an additional game available for purchase. At the start of the game, " +
		...
		return handlerInput.responseBuilder.speak(speechText).reprompt("<voice name='" + voice_name + "'>If you need help, please say 'help'.</voice>").getResponse();
	}
};
const HelpGoodWordHuntingGroupIntent = {
	canHandle(handlerInput) {
		return (
			handlerInput.requestEnvelope.request.type === "IntentRequest" &&
			handlerInput.requestEnvelope.request.intent.name === "HelpGoodWordHuntingGroupIntent"
		);
	},
	handle(handlerInput) {
		...
		let speechText = "<voice name='" + voice_name + "'>Good Word Hunting can also be fun to play in a group setting.  At the start of the game, everyone " +
...
const HelpGetTaglineIntent = {
	canHandle(handlerInput) {
		return (
			handlerInput.requestEnvelope.request.type === "IntentRequest" &&
			handlerInput.requestEnvelope.request.intent.name === "HelpGetTaglineIntent"
		);
	},
	handle(handlerInput) {
		...
		let speechText = "<voice name='" + voice_name + "'>You can use this skill to get the tagline of any movie. Simply say 'get tagline for insert movie'. " +
...
const HelpGetCastIntent = {
	...
	handle(handlerInput) {
		...
		let speechText = "<voice name='" + voice_name + "'>You can use this skill to get the cast list of any movie. Simply say 'who was in insert movie'. " +
	...
	async handle(handlerInput) {
		...
		if (attributes.type) {
			switch (attributes.type) {
				...
				default:
					speechText = "<voice name='" + voice_name + "'>Sorry, I am not sure what you are saying yes for. Would you like some help?</voice>";
					handlerInput.attributesManager.setSessionAttributes({type: "help"});
			}
		}
		else {
		...
		return handlerInput.responseBuilder.speak(speechText).reprompt("<voice name='" + voice_name + "'Would you like some help?</voice>").getResponse();
	}
};
const RepeatIntent = {
	canHandle(handlerInput) {
	...
	handle(handlerInput) {
		...
		if (attributes.repeat) {
			switch (attributes.repeat) {
				...
				case "goodWordHunting":
						let speechText = "<voice name='" + voice_name + "'>The " + attributes.keywords.length + " keywords used to describe this film are <break time='1s'/>";
						for (let i = 0; i < 5; i++) {
							if (i == 4) {
								speechText += " and " +  attributes.keywords[i] + ". <break time='1s'/>";
							}
						...
						speechText += "The " + attributes.numberNamesNeeded + " names from lowest billed to highest are ";
						let castIndex = attributes.cast.length - 1;
						...
						speechText += "With that, I'll give you a few more seconds to think of your answer. <break time='4s'/> Alright, what movie are these keywords and cast members associated with?</voice>";
						handlerInput.attributesManager.setSessionAttributes({
							cast: attributes.cast,
							movie_id: attributes.movie_id,
							movie: attributes.movie,
						...
						.getResponse();
				default:
					speechText = "<voice name='" + voice_name + "'>Sorry, I am not sure what you are wanting me to repeat. Would you like some help?</voice>";
					handlerInput.attributesManager.setSessionAttributes({type: "help"});
			}
		}
		else {
...
const NoIntent = {
	...
	async handle(handlerInput) {
		...
		if (attributes.type) {
			switch (attributes.type) {
				case "whoseTagline":
					speechText = "<voice name='" + voice_name + "'>Okay, if you would like more information about this skill's abilities, please say 'help'</voice>";
					break;
				case "goodWordHuntingStart":
					await handlerInput.attributesManager.setSessionAttributes({ 
						type: "goodWordHunting",
					...
					return StartGameIntent.handle(handlerInput);
				default:
					speechText = "<voice name='" + voice_name + "'>Sorry, I am not sure what you are saying no for. Would you like some help?</voice>";
					handlerInput.attributesManager.setSessionAttributes({type: "help"});
			}
		}
		else {
...
const Fallback = {
	...
	handle(handlerInput) {
		...
		return handlerInput.responseBuilder
			.speak("<voice name='" + voice_name + "'>Sorry, I didn\'t understand what you said. Please try again.</voice>")
			.reprompt("<voice name='" + voice_name + "'>Sorry, I didn\'t understand what you said. Please try again.</voice>")
			.getResponse();
	}
};
const ShopIntent = {
	...
	handle(handlerInput) {
		...
		return handlerInput.responseBuilder
		.speak("<voice name='" + voice_name + "'>If you enjoyed playing Whose Tagline Is It Anyway, you may also be interested in Good Word Hunting. " +
		"For more information please say, 'what is Good Word Hunting'.</voice>")
		.reprompt("<voice name='" + voice_name + "'>For more information please say, 'what is Good Word Hunting'.</voice>")
		.getResponse();
	}
};
const BuyIntent = {
		...
		return ms.getInSkillProducts(locale).then(function handlePurchaseResponse(result) {
			...
			if (handlerInput.requestEnvelope.request.status.code === '200') {
				...
				switch (handlerInput.requestEnvelope.request.payload.purchaseResult) {
					case 'ACCEPTED':
						speakOutput = "<voice name='" + voice_name + "'>Thank you for your purchase! You now have full access to Good Word Hunting. Would you like to play a game right now?</voice>";
						repromptOutput = "<voice name='" + voice_name + "'>Would you like to play a game right now?</voice>";
						handlerInput.attributesManager.setSessionAttributes({ type: "goodWordHunting" });
						break;
					case 'DECLINED':
						speakOutput = "<voice name='" + voice_name + "'>Thank you for your interest in buying Good Word Hunting. Shall we continue playing Whose Tagline Is It Anyway?</voice>";
						repromptOutput = "<voice name='" + voice_name + "'>Shall we continue playing Whose Tagline Is It Anyway?</voice>";
						handlerInput.attributesManager.setSessionAttributes({ type: "whoseTagline" });
						break;
					case 'ALREADY_PURCHASED':
						speakOutput = "<voice name='" + voice_name + "'>It appears you already have access to play Good Word Hunting. Would you like to play a game right now?</voice>";
						repromptOutput = "<voice name='" + voice_name + "'>Would you like to play a game right now?</voice>";
						handlerInput.attributesManager.setSessionAttributes({ type: "goodWordHunting" });
						break;
					default:
						console.log(`unhandled purchaseResult: ${handlerInput.requestEnvelope.payload.purchaseResult}`);
						speakOutput = "<voice name='" + voice_name + "'>Something unexpected happened during your purchase, thank you for your interest in buying Good Word Hunting. " +
							"Perhaps re-try the transaction in a bit. Shall we continue playing Whose Tagline Is It Anyway in the meantime?</voice>";
						repromptOutput = "<voice name='" + voice_name + "'>Shall we continue playing Whose Tagline Is It Anyway in the meantime?</voice>";
						break;
				}
				return handlerInput.responseBuilder
					.speak(speakOutput)
		...
		return ms.getInSkillProducts(locale).then(function handleCancelResponse(result) {
			...
			if (handlerInput.requestEnvelope.request.status.code === '200') {
				if (handlerInput.requestEnvelope.request.payload.purchaseResult === 'ACCEPTED') {
					const speechText = "<voice name='" + voice_name + "'>Your request to refund Good Word Hunting has been processed. Would you like to resume playing Whose Tagline Is It Anyway instead?</voice>";
					const repromptText = "<voice name='" + voice_name + "'>Would you like to play a game of Whose Tagline Is It Anyway?</voice>";
					handlerInput.attributesManager.setSessionAttributes({ type: "whoseTagline" });
					return handlerInput.responseBuilder
						.speak(speechText)
				...
				if (handlerInput.requestEnvelope.request.payload.purchaseResult === 'NOT_ENTITLED') {
					const speechText = "<voice name='" + voice_name + "'>Sorry, you don't seem to have any purchases available for a refund. This skill has one game available for purchase called Good Word Hunting. Would you like to hear about it?</voice>";
					const repromptText = "<voice name='" + voice_name + "'>Would you like to hear about Good Word Hunting?</voice>";
					handlerInput.attributesManager.setSessionAttributes({ type: "goodWordHuntingHelp" });
					return handlerInput.responseBuilder
						.speak(speechText)
						.reprompt(repromptText)
...
const PurchasedIntent = {
	...
	handle(handlerInput) {
		...
		return ms.getInSkillProduct(locale, productId).then(function (result) {
			if (result.entitled == 'ENTITLED') {
				// set game type to good word hunting, call start game intent
				let speechText = "<voice name='" + voice_name + "'>You have purchased the rights to play Good Word Hunting. Would you like to play a quick game?</voice>";
				let repromptText = "<voice name='" + voice_name + "'>Would you like to play a game of Good Word Hunting?</voice>";
				handlerInput.attributesManager.setSessionAttributes({ type: "goodWordHuntingStart" });
				return handlerInput.responseBuilder.speak(speechText).reprompt(repromptText).getResponse();
			}
			else {
				// user not eligible to play, ask if they would like to hear game description
				let speechText = "<voice name='" + voice_name + "'>You have not made any in-skill purchases.</voice>";
				handlerInput.attributesManager.setSessionAttributes({ type: "goodWordHuntingHelp" });
...
const UnhandledHandler = {
	...
	handle(handlerInput, error) {
		let speechText = "<voice name='" + voice_name + "'>Sorry, I am not quite sure what to do.</voice>";
