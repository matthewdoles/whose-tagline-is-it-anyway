  ...
  handle: async function handle(handlerInput) {
    ...
    try {
      ...
      if (movie.results.length > 0) {
          ...
          if (credits.cast.length != 0) {
            ...
            if (credits.cast.length < 10) {
              ...
              for (var i = 0; i < credits.cast.length; i++) {
                if (i + 1 == credits.cast.length) {
                  speechText += 'and ' + credits.cast[i].name + ". <break time='1s'/>";
                } else {
            ...
            // if movie has less than 10 credits, name them all from highest billed to lowest
                ...
                for (var _i = 0; _i < 10; _i++) {
                  if (_i + 1 == 10) {
                    speechText += 'and ' + credits.cast[_i].name + ". <break time='1s'/>";
                  } else {
        ...
        speechText = VOICE_OPEN + 'Sorry, I was not able to find a match for ' + movieInput + '. Try including the year after the title by saying ' + "get the cast for 'insert movie' from 'insert year'. ";
      }
    } catch (error) {
      speechText = VOICE_OPEN + MOVIEDB_ERROR + 'Please try again.' + VOICE_CLOSE;
    }
    var closingText = "If you would like to get the cast for another movie, please say get cast for 'insert movie'." + VOICE_CLOSE;
    speechText += closingText;

    return handlerInput.responseBuilder.speak(speechText).reprompt(VOICE_OPEN + closingText).getResponse();
  }
};