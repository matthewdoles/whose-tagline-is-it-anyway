...
var HintIntent = exports.HintIntent = {
  ...
  handle: async function handle(handlerInput) {
    ...
    if (attributes.type == 'whoseTagline' || attributes.type == 'answer') {
      ...
      // second hint, movie top two billed cast members, make api call
          ...
          try {
            ...
            speechText = VOICE_OPEN + "The top two billed people for this movie are <break time='1s'/>'" + movie.cast[0].name + "' and '" + movie.cast[1].name + "' <break time='1s'/>. With that, I'll give you a few more seconds to think about it." + " <break time='4s'/> Okay, that was the last hint I can give you, " + 'what would you like to do? Repeat the tagline or answer.</voice>';
          } catch (error) {
    ...
    if (attributes.type == 'goodWordHunting') {
      ...
      speechText = VOICE_OPEN + "Sorry, I can't give you any hints, but I'll give you a " + 'few more seconds to think of your answer.' + answerPompt;
      VOICE_CLOSE;
      ...
      if (easterEgg == 3) {
        speechText = VOICE_OPEN + 'Okay I give in, this movie came out in ' + _attributes.year + ". I hope that helps, I'll give you a few more seconds to think of your answer." + answerPompt + VOICE_CLOSE;
      }
