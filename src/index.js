...
const StartGameIntent = {
  ...
  async handle(handlerInput) {
    ...
    if (attributes.type == 'whoseTagline') {
      try {
        let random_movie_tagline = await MovieFunctions.getMovieTagline(
          movie_id
        );
    ...
    // if game is good word hunting, make api call for keywords then cast members
      ...
      try {
        let random_movie_keywords = await MovieFunctions.getMovieKeywords(
          movie_id
        );
        let keywords = [];
        if (random_movie_keywords.keywords.length != 0) {
          ...
          try {
            let random_movie_credits = await MovieFunctions.getMovieCredits(
              movie_id
            );
  ...
  async handle(handlerInput) {
    ...
    if (attributes.type == 'whoseTagline' || attributes.type == 'answer') {
      ...
      // second hint, movie top two billed cast members, make api call
        ...
        try {
          let movie_credits = await MovieFunctions.getMovieCredits(
            attributes.movie_id
          );
  ...
  async handle(handlerInput) {
    ...
    try {
      ...
      if (search_movie.results.length > 0) {
        ...
        try {
          let search_movie_tagline = await MovieFunctions.getMovieTagline(
            movie_id
          );
  ...
  async handle(handlerInput) {
    ...
    try {
      ...
      if (search_movie.results.length > 0) {
        ...
        try {
          let search_movie_credits = await MovieFunctions.getMovieCredits(
            movie_id
          );
